 ...
 * Created on October 20, 2022, 9:31 am
 ...
 *          Check the input of both player and computer that hit successfully or not, update both player matrix and ship status structure
...
int displayShipStatus(const BattleShip&, const BattleShip&); // Draw the ship status and return a integer indicating the winning status
...
void showPlayZone(char** playerMatrix, char** pcMatrix) {
    ...
    cout << "~~'*'=unexplore area ~~ 'o'=unsuccessful attack ~~ '@'=successful attack ~~" << endl;
    cout << "....................................................................." << endl;
...
int displayShipStatus(const BattleShip& player, const BattleShip& computer)
{
    ...
    cout << "...................................................................." << endl;
    cout << " -      Computer Ships Status                 Your Ship Status-" << endl;                      //Display the ship status
    cout << "...................................................................." << endl;
    cout << "----Aircraft: " << airCarftComputer << " units                 | ----Aircraft: " << airCarftPlayer << " units" << endl;
    cout << "----Battleship: " << battelShipComputer << " units               | ----Battleship: " << battelShipPlayer << " units" << endl;
    cout << "----Destroyer: " << destroyerComputer << " units                | ----Destroyer: " << destroyerPlayer << " units" << endl;
    cout << "----Corvette: " << corvetteComputer << " units                 | ----Corvette: " << corvettePlayer << " units" << endl;
    cout << "....................................................................." << endl;
    if (airCarftComputer == 0 && battelShipComputer == 0 && destroyerComputer == 0 && corvetteComputer == 0) // Check all computer ships were hit successfully
    {
        return 0;   // Computer loss and player win
    }
    else if (airCarftPlayer == 0 && battelShipPlayer == 0 && destroyerPlayer == 0 && corvettePlayer == 0)   // Check all player ships were hit successfully
    {
    ...
    while (true)                                        // Loop for setup the aircraft position    
    {
        ...
        if (tolower(shipOrientiation) == 'h')                                     // check for horizontal setup
        {
            ...
            else
            {
                ...
                {
                    playerMatrix[startPositionRow][i] = 'A';                          // set the player matrix with 'A' for indicationg the aircraft location
    ...
    while (true)                                        // loop for setup the battleship position    
    {
        ...
        if (tolower(shipOrientiation) == 'h')                               // check for horizontal setup
        {
            ...
            else
            {
                ...
                {
                    playerMatrix[startPositionRow][i] = 'B';                          // set the player matrix with 'B' for indicationg the battle location
    ...
    while (true)                                        // Loop for setup the battleship position    
    {
        ...
        if (tolower(shipOrientiation) == 'h')                               // check for horizontal setup
        {
            ...
            else
            {
                ...
                {
                    playerMatrix[startPositionRow][i] = 'D';                          // set the player matrix with 'D' for indicationg the battle location
...
void startPlay(string playerName, char** playerMatrix, char** pcMatrix, BattleShip* battleShipInfo)
{
    ...
    while (true) {
        while (true) {                                      // Take a infinite loop for satisfying the valid input for attack position
            ...
            if (attackPosition.length() == POSITION_LENGTH) { // position length should the 2 character length
                ...
                if ((attackPosition[0] >= 'A' && attackPosition[0] <= 'J') && (attackPosition[1] >= '0' && attackPosition[1] <= '9')) { // check for valid input
                    ...
                    isSuccessful = checkSuccessful(pcMatrix, &battleShipInfo[1], rowPosition, colPosition); // Check it is successfully hit or not
                    if (isSuccessful) {                         // If successfully hit then player will get another change for attack
                        ...
                        gameHead(playerName);                   // Draw the game head
                        showPlayZone(playerMatrix, pcMatrix);   // Draw the play zone
                        cout << "You miss the hit. Now computer's turn! " << endl;
        ...
        {
            cout << "Congratulation!!! ~~~" << playerName << "~~~ You won this game!!!" << endl;
            break;
        }
        while (true) {                                      // Take a infinite loop for satisfying the valid input for attack position
            ...
            isSuccessful = checkSuccessful(playerMatrix, &battleShipInfo[0], rowPosition, colPosition); // Check if it is successfully hit or not 
            if (isSuccessful) {                         // If successfully hit then player will get another change for attack
                trackWin = displayShipStatus(battleShipInfo[0], battleShipInfo[1]);
                gameHead(playerName);                   // Draw the game head
                showPlayZone(playerMatrix, pcMatrix);   // Draw the play zone
                cout << "Computer attack the position: " << yCoord[rowPosition] << colPosition << endl;
        ...
        {
            cout << "Congratulation!!! ~~~ Computer ~~~ won this game!!!" << endl;
            break;
        }
...
//This function check the attack is successful or not and update the ship position structure  *
//*****************************************************************
bool checkSuccessful(char** playerMatrix, BattleShip *shipPosition, int row, int col) 
{
    ...
    for (int i = 0; i < AIRCRAFT_LENGTH; i++)           // Check the air craft position
    {
        if (shipPosition->aircraft[i][0] != -1)         // If the position not hit yet
        {
            if (shipPosition->aircraft[i][0] == row && shipPosition->aircraft[i][1] == col) //hit successful
            {
                shipPosition->aircraft[i][0] = -1;   // Track this position destroy
    ...
    for (int i = 0; i < BATTLESHIP_LENGTH; i++)     // Check the battle ship position
    {
        if (shipPosition->battleship[i][0] != -1)   // If the position not hit yet
        {
            if (shipPosition->battleship[i][0] == row && shipPosition->battleship[i][1] == col) //hit successful
            {
                shipPosition->battleship[i][0] = -1;   // Track this position destroy
                playerMatrix[row][col] = '@';        // Set the matrix position '@' if hit successfully
                isFound = true;                     // The value is found already
                return isFound;
            }
    ...
    for (int i = 0; i < DESTROYER_LENGTH; i++)     // Check the Destroyer position
    {
        if (shipPosition->destroyer[i][0] != -1)   // If position not hit yet 
        {
            if (shipPosition->destroyer[i][0] == row && shipPosition->destroyer[i][1] == col) //hit successful
            {
                shipPosition->destroyer[i][0] = -1;   // Track this position destroy
                playerMatrix[row][col] = '@';        // Set the matrix position '@' if hit successfully
                isFound = true;                     // The value is found already
                return isFound;
            }
    ...
    for (int i = 0; i < CORVETTE_LENGTH; i++)     // Check the corvette position
    {
        if (shipPosition->corvette[i][0] != -1)   // If position not hit yet 
        {
            if (shipPosition->corvette[i][0] == row && shipPosition->corvette[i][1] == col) //hit successful
            {
                shipPosition->corvette[i][0] = -1;   // Track this position destroy
                playerMatrix[row][col] = '@';        // Set the matrix position '@' if hit successfully
                isFound = true;                     // The value is found already
                return isFound;
            }
