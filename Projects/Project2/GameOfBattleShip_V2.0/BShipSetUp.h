...
class BShipSetUp {    // Declare a class for different types battle ship set up
    private:
        ...
        bool conflictWithOtherShip(char**, int, int, int, char); 
    ...
    public:
        ...
        BShipSetUp setYourBattleShip(char**); // mutator function,and set player ship position and return structure
        //Determine the letter (A,...J) value to integer y-axis value (0,1,2...9) using enum identifier 
        friend int letterToRowNumber(char); 
        void drawPlayerArea(const char* const*, const BattleShip&); //Display player matrix     
