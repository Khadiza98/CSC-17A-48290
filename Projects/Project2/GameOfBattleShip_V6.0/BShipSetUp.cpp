...
//*****************************************************************
//Definition of function startPlay. This function allow to                    
// input player and computer attack position each other 
//*****************************************************************
void BShipSetUp::startPlay(BShipSetUp battleShipPlayer, BShipSetUp
    ...
    while (true) {
        ...
        if (trackWin == 0) { // The player won
            ...
            *winner = 'p';     // Hold player win the game
            break;
        } // end if
        while (true) { // Take a infinite loop for satisfying the valid input for attack position
        ...
        if (trackWin == 1) { // The computer won
            ...
            *winner = 'c';                 // Hold the winner 
            break;
        }//end if
    } //End while-loop
    dt.setEndTimeOfPlay(currentTime());  //Hold the end time of play
    if (attackPosition == "S") {       // If save the game
        *winner = 's';                  // No one win the game, and is saved game
        GameSummary objGSummary(attackCount, winner, &dt);    // Allocate memory for single structure
...
void BShipSetUp::saveGame(BShipSetUp player, BShipSetUp computer, 
    ...
    char pMat[10][10]; //declare vector for player matrix
    char cMat[10][10]; // decaler vector for computer matrix
    ...
    for (int i = 0; i < 5; i++)
        for (int j = 0; j < 2; j++)
            pShip << player.aircraft[i][j] << ";";
    pShip << endl;
    for (int i = 0; i < 4; i++)
        for (int j = 0; j < 2; j++)
            pShip << player.battleship[i][j] << ";";
    pShip << endl;
    for (int i = 0; i < 3; i++)
        for (int j = 0; j < 2; j++)
            pShip << player.destroyer[i][j] << ";";
    pShip << endl;
    for (int i = 0; i < 2; i++)
        for (int j = 0; j < 2; j++)
            pShip << player.corvette[i][j]<< ";";
    pShip << endl;
...
//and string parameters.This friend function check file open in or not. 
